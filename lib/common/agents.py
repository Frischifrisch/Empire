...
class Agents:
    ...
    def add_agent(self, sessionID, externalIP, delay, jitter, profile, killDate, workingHours, lostLimit, sessionKey=None, nonce='', listener='', language=''):
        ...
        try:
            ...
            dispatcher.send(signal, sender=f"agents/{sessionID}")
    ...
    def remove_agent_db(self, sessionID):
        ...
        try:
            ...
            # dispatch this event
            message = f"[*] Agent {sessionID} deleted"
            signal = json.dumps({
                'print': True,
                'message': message
            })
    ...
    def clear_agent_tasks_db(self, sessionID):
        ...
        message = f"[*] Tasked {sessionID} to clear tasks"
        signal = json.dumps({
            'print': True,
            'message': message
        })
    ...
    def handle_agent_data(self, stagingKey, routingPacket, listenerOptions, clientIP='0.0.0.0', update_lastseen=True):
        ...
        if len(routingPacket) < 20:
            message = f"[!] handle_agent_data(): routingPacket wrong length: {routingPacket}"
            signal = json.dumps({
                'print': False,
                'message': message
            })
        ...
        for sessionID, (language, meta, additional, encData) in routingPacket.iteritems():
            ...
            if meta in ['STAGE0', 'STAGE1', 'STAGE2']:
                ...
                message = f"[*] handle_agent_data(): sessionID {sessionID} issued a RESULT_POST"
                signal = json.dumps({
                    'print': False,
                    'message': message
                })

...
class Flags(object):
    ...
    def __setitem__(self, key, value):
        if not self._flags.has_key(key):
            raise KeyError(f"The key '{key}' is not defined for those flags.")
    ...
    def __init__(self, root):
        ...
        if root in _ROOT_LOCATION_GUIDS.keys():
            ...
            if len(bytes) == 18: # and bytes[:2] == '\x1F\x50':
                ...
                bytes = bytes[2:]
            if len(bytes) != 16:
                raise FormatException(f"This is no valid _GUID: {bytes}")
...
class DriveEntry(object):
    ...
    def __str__(self):
        return f"<DriveEntry: {self.drive}>"
...
class PathSegmentEntry(object):
    ...
    def __str__(self):
        return f"<PathSegmentEntry: {self.full_name}>"
...
class Lnk(object):
    ...
    def _write_hot_key(self, hot_key, lnk):
        if hot_key is None:
            ...
            try:
                ...
                raise InvalidKeyException(f"Cannot find key code for {hot_key[1]}")
    ...
    def save(self, f=None, force_ext=False):
        ...
        if not is_file:
            if type(f) not in [str, unicode]:
                raise ValueError(f"Need a writeable object or a file name to save to, got {f}")
            if force_ext:
    ...
    def _set_window_mode(self, value):
        if value not in _SHOW_COMMANDS.values():
            raise ValueError(
                f"Not a valid window mode: {value}. Choose any of pylnk.WINDOW_*"
            )

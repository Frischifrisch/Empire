...
globentropy = random.randint(1, datetime.now().day)
globDebug=False
###############################################################
#
# Validation methods
...
def get_dependent_functions(code, functionNames):
    ...
    dependentFunctions = {
        ...
        if re.search(
            f"[^A-Za-z']+{functionName}[^A-Za-z']+", code, re.IGNORECASE
        )
    }
...
def get_config(fields):
    ...
    for field in fields.split(','):
        if field.strip() not in columns:
            cur.execute(f"ALTER TABLE config ADD COLUMN {field} BLOB")

    cur.execute(f"SELECT {fields} FROM config")

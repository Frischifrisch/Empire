...
class CFinder(object):
    ...
    def _get_info(self, repoName, fullname):
        ...
        msg = f'Unable to locate module {submodule} in the {repoName} repo'
...
def get_job_message_buffer():
    ...
    try:
        ...
        return build_response_packet(0, f"[!] Error getting job output: {e}")
...
def permissions_to_unix_name(st_mode):
    ...
    for usertype in usertypes:
        ...
        for permtype in perm_types:
            perm = getattr(stat, f'S_I{permtype}{usertype}')
...
def directory_listing(path):
    ...
    for fn in os.listdir(path):
        ...
        permstr = f"d{permstr}" if os.path.isdir(fn) else f"-{permstr}"
...
def run_command(command, cmdargs=None):
    ...
    if re.compile("(ls|dir)").match(command):
        ...
        os.mkdir(cmdargs)
        return f"Created directory: {cmdargs}"
